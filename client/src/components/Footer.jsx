import { assets, footerLinks } from "../assets/assets";

const Footer = () => {
    
    // Platform Specific Data for refined iconography and branding
    const socialPlatforms = [
        { 
            name: "Linkedin", 
            url: "https://www.linkedin.com/in/ayushlohiya/", 
            color: "hover:bg-[#0077B5] hover:border-[#0077B5]",
            icon: (
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.238 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
            )
        },
        { 
            name: "Github", 
            url: "https://github.com/9140ayush", 
            color: "hover:bg-[#24292e] hover:border-[#24292e]",
            icon: (
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.042-1.416-4.042-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                </svg>
            )
        },
        { 
            name: "Instagram", 
            url: "#", 
            color: "hover:bg-gradient-to-tr hover:from-[#f9ce34] hover:via-[#ee2a7b] hover:to-[#6228d7] hover:border-transparent",
            icon: (
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
            )
        },
        { 
            name: "YouTube", 
            url: "#", 
            color: "hover:bg-[#FF0000] hover:border-[#FF0000]",
            icon: (
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"/>
                </svg>
            )
        }
    ];
    
    return (
        <footer className="mt-32 border-t border-border-main bg-page/50 dark:bg-surface/30">
            <div className="px-6 md:px-16 lg:px-24 xl:px-32">
                <div className="flex flex-col md:flex-row items-start justify-between gap-16 py-20 border-b border-border-soft">
                    <div className="max-w-md">
                        <div className="mb-8" style={{
                            fontFamily: 'var(--font-display)',
                            fontSize: '32px',
                            fontWeight: 700,
                            letterSpacing: '-0.02em',
                            lineHeight: 1,
                        }}>
                            <span style={{ color: '#3BB77E' }}>Green</span><span style={{ color: 'var(--text-heading)' }}>Cart</span>
                        </div>
                        <p className="text-body font-medium leading-relaxed opacity-80">
                            We deliver farm-fresh groceries and gourmet snacks straight to your door. Trusted by thousands of families, we're on a mission to make healthy shopping effortless, premium, and affordable.
                        </p>
                        
                        <div className="flex gap-4 mt-8">
                            {socialPlatforms.map((platform, index) => (
                                <a 
                                    key={index}
                                    href={platform.url}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`w-12 h-12 rounded-2xl bg-surface dark:bg-card border border-border-main flex items-center justify-center cursor-pointer transition-all duration-300 group ${platform.color} active:scale-95`}
                                    aria-label={platform.name}
                                >
                                    <span className="text-muted group-hover:text-white transition-colors">
                                        {platform.icon}
                                    </span>
                                </a>
                            ))}
                        </div>
                    </div>

                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-12 w-full md:w-auto">
                        {footerLinks.map((section, index) => (
                            <div key={index} className="min-w-[140px]">
                                <h3 className="font-black text-heading uppercase tracking-widest text-xs mb-8">{section.title}</h3>
                                <ul className="space-y-4">
                                    {section.links.map((link, i) => (
                                        <li key={i}>
                                            <a href={link.url} className="text-body font-medium hover:text-accent transition-all flex items-center gap-2 group">
                                                <span className="w-1.5 h-1.5 rounded-full bg-accent opacity-0 group-hover:opacity-100 transition-opacity"></span>
                                                {link.text}
                                            </a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ))}
                    </div>
                </div>

                <div className="py-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <p className="text-sm font-bold text-muted uppercase tracking-widest">
                        © {new Date().getFullYear()} GreenCart • Handcrafted by Ayush.dev
                    </p>
                    <div className="flex gap-8">
                        <span className="text-xs font-bold text-muted hover:text-accent cursor-pointer transition-colors uppercase tracking-widest">Privacy Policy</span>
                        <span className="text-xs font-bold text-muted hover:text-accent cursor-pointer transition-colors uppercase tracking-widest">Terms of Service</span>
                    </div>
                </div>
            </div>
        </footer>
    );
};

export default Footer;